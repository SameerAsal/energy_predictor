...
int papi_events[3] = {PAPI_L1_DCM, PAPI_L2_TCM, PAPI_TOT_INS}; \
long_long CtrValues[6]; \
\
...
if (PAPI_query_event(PAPI_TOT_INS) == PAPI_OK) { \
    printf("  * PAPI will count number of instructions completed\n"); \
    err = PAPI_add_event(EventSet, papi_events[2]); \
} \
if (err != PAPI_OK) { \
    printf("Failed to add PAPI event\n"); \
    exit(1); \
...
#define PERF_EXIT \
        ...
        printf("Total Instructions completed: %ld\n", CtrValues[2]);
#endif
